...
public class WhenGettingCourseProviders
{
        ...
        int page, Guid shortlistUserId)
    {
        ...
        if (page > 1) pageParam = $"&page={page}&pageSize=10";

        //Assert
        actual.GetUrl.Should().Be($"{baseUrl}courses/{id}/providers?orderBy={orderBy}&distance={distance}&location={location}&deliveryModes={string.Join("&deliveryModes=", deliveryModeTypes)}&employerProviderRatings={string.Join("&employerProviderRatings=", employerProviderRatingTypes)}&apprenticeProviderRatings={string.Join("&apprenticeProviderRatings=", apprenticeProviderRatingTypes)}&qar={string.Join("&qar=", qarRatings)}{pageParam}&shortlistUserId={shortlistUserId}");
    }
