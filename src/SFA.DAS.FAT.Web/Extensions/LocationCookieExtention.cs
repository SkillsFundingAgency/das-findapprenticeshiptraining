...
{
    public static (string LocationName, string Distance) GetLocation(
        this ICookieStorageService<LocationCookieItem> cookieService)
    {
        ...
        return (locationCookieItem?.LocationName ?? string.Empty, locationCookieItem?.Distance ?? DistanceService.DefaultDistance.ToString());
    }
}
